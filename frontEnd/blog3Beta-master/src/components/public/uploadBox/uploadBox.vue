        ...
        <div v-bind:style="{height: `calc(${height} - 64px - 32px)`, minHeight: 'calc(190px - 64px)', maxHeight: max_height}">
            <ul class="article_upload_box_fileArea">
                ...
                <li v-for="i in file_list_data_arr">
                    <!-- 此处显示文件地址链接 -->
                    <p>
                        <input :value="i.file_url.replace(/\$_SERVER_IP/, host_path)"/>
                    </p>
                    <div>
                        <!-- left预览图片如果有 -->
                        <div>
                            <img @click="openFileNewTab(i.file_url.replace(/\$_SERVER_IP/, host_path))" :src="i.file_url.replace(/\$_SERVER_IP/, host_path) || ''"/>
                        </div>
                        <!-- right 文件信息 -->
                        <div>
                            <!-- top文件名 -->
...
export default {
    ...
    mounted () {
        ...
        this.host_path = window.location.host
    }
}
</script>
